...
async def run_full_diagnostics():
    ...
    if redis_host:
        ...
        print("   ❌ ERROR: REDIS_HOST is missing. Check your .env file.")
        return
    ...
    try:
        response = await service.process_query(
            AIQueryDTO(text=query_text, project_id="1")
        )
        print("   ✅ Pipeline Success!")
        print(
            f"      - Intent detected: {response.intent.label if response.intent else 'None'}"
        )
        print(f"      - Final Source: {response.source}")

        if response.source == "llm_hybrid":
            print(
                "      (Note: It fell through to Layer 3 because Redis didn't have a STATIC match. This is normal behavior.)"
            )
    ...
    service.router.get_intent = MagicMock(
        return_value={"intent": "STATIC", "confidence": 0.40}
    )

    response = await service.process_query(
        AIQueryDTO(text="Unsure query", project_id="1")
    )

    if response.source == "system" and "clarify" in response.answer:
        print(
            "   ✅ Ambiguity Caught: System asked for clarification (Confidence: 0.40)."
        )
    else:
    ...
    response = await service.process_query(
        AIQueryDTO(text="Crash test", project_id="1")
    )

    if response.source == "hybrid_rag" or response.intent.label == "dynamic":
        print(
            "   ✅ Circuit Breaker Active: Router crash failed open to Hybrid Search."
        )
    else:
